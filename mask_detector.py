...
model = tf.keras.models.load_model('mask_detector_mobilenetv2.model')
...
while True:
    ...
    for face in faces:
        ...
        if mask_probability > 70:
            label = "No Mask {:.2f}%".format(mask_probability)
            color = (0, 0, 255)  # Red for no mask
        else:
            label = "Mask {:.2f}%".format(100 - mask_probability)
